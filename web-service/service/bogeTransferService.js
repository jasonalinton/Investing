...
class BogeTransferService {
        ...
        if (self.processing == false) {
            ...
            self.getLastSavedTransferTime()
                .then((res) => new Promise((resolve) => {
                    let lastSavedTime = new Date(res.data.data.getLastSavedTransferTime);
                    console.log(`Boge history last saved on ${lastSavedTime.toJSON()}`);

                    self.fetchTranfers(self, lastSavedTime, resolve);
                }))
                .then(() => { 
    ...
    async fetchTranfers(self, lastSavedTransferTime, resolve) {
        ...
        axios.post("https://graphql.bitquery.io", data)
            .then((res) => {
                self.createTransfer(self, lastSavedTransferTime, res);
